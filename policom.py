...
class pico: #for 2 fares only
    def __init__(me, n, b, V, R, f, p, kk=1):
        ...
        dd,dp = float(kk), p[1]-p[0]
        #def pcom(i,j): return p[0]+dp*(i/dd), p[0]+dp*(j/dd)
        #me.nr = [pcom(i,j) for i in range(kk+1) for j in range(kk+1)
        #         if  i in (0,kk) or j in (0,kk)]
        me.nr = [(p[0]+dp*(i/dd), p[0]) for i in range(kk)]
        me.nr+= [(p[1], p[0]+dp*(i/dd)) for i in range(kk)]
        me.nr+= [(p[1]-dp*(i/dd), p[1]) for i in range(kk)]
        me.nr+= [(p[0], p[1]-dp*(i/dd)) for i in range(kk)]
    ...
    def offrev(me, s, r): #offline optimal
        w = [0.,0.]
        left = me.n
        #assume high fare is more valuable
        for i in range(0,2): 
        ...
        w = [0.,0.]
        left = me.n
        #assume low fare is more valuable
...
ex2b = (13, 1, 7, 3, 10, 0.10, 2.7, 0.9, 0.10, 0.32)
ex2c = (13, 1, 7, 3, 10, 0.10, 3.7, 0.9, 0.1, 0.3)
...
if __name__ == "__main__":
    ...
    kk =20 
    #randmip(20, kk)
    exs = (ex1, ex2c, ex2b, ex2, ex3, ex4, 
     ex5, ex6, ex7, ex8, ex9, 
      ex11, ex12, ex12b)
    for i in range(len(exs)):
        n, ll, lh, hl, hh, beta, V, f2, p0, p1 = exs[i]
        mp = pico(n, beta, V, ((hl, hh), (ll, lh)), (1.,f2), (p0,p1), kk)
        if checkmip(mp) > 0: 
           testSample(exs[i], kk); print(i); break
